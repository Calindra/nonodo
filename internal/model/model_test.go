...
func (s *ModelSuite) SetupTest() {
	...
	for i := 0; i < s.n; i++ {
		...
		s.payloads[i] = string([]byte{0xf0 + byte(i)})
...
func (s *ModelSuite) TestItAddsAndGetsInspectInput() {
	// add inputs
	for i := 0; i < s.n; i++ {
		index := s.m.AddInspectInput([]byte(s.payloads[i]))
		s.Equal(i, index)
	}
	...
	for i := 0; i < s.n; i++ {
		...
		s.Equal(i, input.Index)
		s.Equal(cModel.CompletionStatusUnprocessed, input.Status)
		s.Equal([]byte(s.payloads[i]), input.Payload)
...
func (s *ModelSuite) TestItGetsFirstInspectInput() {
	// add inputs
	for i := 0; i < s.n; i++ {
		s.m.AddInspectInput([]byte(s.payloads[i]))
	}
	...
	s.True(ok)
	s.Equal(0, convertedInput.Index)
	s.Equal([]byte(s.payloads[0]), convertedInput.Payload)
}
...
func (s *ModelSuite) TestItFinishesAdvanceWithAccept() {
	...
	s.NoError(err)
	_, err = s.m.AddVoucher(common.HexToAddress(devnet.ApplicationAddress), s.senders[0], "0", []byte(s.payloads[0]))
	s.NoError(err)
	_, err = s.m.AddNotice([]byte(s.payloads[0]), common.HexToAddress(devnet.ApplicationAddress))
	s.NoError(err)
	err = s.m.AddReport(common.HexToAddress(devnet.ApplicationAddress), []byte(s.payloads[0]))
	s.NoError(err)
...
func (s *ModelSuite) TestItFinishesInspectWithAccept() {
	...
	s.m.AddInspectInput([]byte(s.payloads[0]))
	_, err := s.m.FinishAndGetNext(true) // get
	s.NoError(err)
	err = s.m.AddReport(common.HexToAddress(devnet.ApplicationAddress), []byte(s.payloads[0]))
	s.NoError(err)
	...
	s.Equal(cModel.CompletionStatusAccepted, input.Status)
	s.Equal([]byte(s.payloads[0]), input.Payload)
	s.Equal(0, input.ProcessedInputCount)
...
func (s *ModelSuite) TestItAddsVoucher() {
	...
	// add vouchers
	for i := 0; i < s.n; i++ {
		index, err := s.m.AddVoucher(common.HexToAddress(devnet.ApplicationAddress), s.senders[i], "0", []byte(s.payloads[i]))
		s.Nil(err)
		s.Equal(i, index)
	}
	...
	for i := 0; i < s.n; i++ {
		...
		s.Equal([]byte(s.payloads[i]), common.Hex2Bytes(vouchers.Rows[i].Payload[2:]))
	}
}
...
func (s *ModelSuite) TestItAddsNotice() {
	...
	// add notices
	for i := 0; i < s.n; i++ {
		index, err := s.m.AddNotice([]byte(s.payloads[i]), common.HexToAddress(devnet.ApplicationAddress))
		s.Nil(err)
		s.Equal(i, index)
	}
	...
	for i := 0; i < s.n; i++ {
		...
		s.Equal([]byte(s.payloads[i]), common.Hex2Bytes(notices.Rows[i].Payload[2:]))
	}
}
...
func (s *ModelSuite) TestItAddsReportWhenAdvancing() {
	...
	// add reports
	for i := 0; i < s.n; i++ {
		err := s.m.AddReport(common.HexToAddress(devnet.ApplicationAddress), []byte(s.payloads[i]))
		s.Nil(err)
	}
...
func (s *ModelSuite) TestItAddsReportWhenInspecting() {
	...
	for i := 0; i < s.n; i++ {
		...
		s.Equal([]byte(s.payloads[i]), reports.Reports[i].Payload)
	}
}
...
func (s *ModelSuite) TestItRegistersExceptionWhenAdvancing() {
	...
	err = s.m.AddReport(common.HexToAddress(devnet.ApplicationAddress), []byte(s.payloads[0]))
	s.Nil(err)
	err = s.m.RegisterException([]byte(s.payloads[0]))
	s.Nil(err)
	...
	s.Empty(input.Notices)
	s.Empty(input.Reports)
	s.Equal([]byte(s.payloads[0]), input.Exception)
...
func (s *ModelSuite) TestItGetsVoucher() {
	...
	for i := 0; i < s.n; i++ {
		...
		s.NoError(err)
		for j := 0; j < s.n; j++ {
			_, err := s.m.AddVoucher(common.HexToAddress(devnet.ApplicationAddress), s.senders[j], "0", []byte(s.payloads[j]))
			s.Nil(err)
		}
		_, err = s.m.FinishAndGetNext(true) // finish
		s.NoError(err)
	...
	for i := 0; i < s.n; i++ {
		for j := 0; j < s.n; j++ {
			...
			s.Equal([]byte(s.payloads[j]), common.Hex2Bytes(voucher.Payload[2:]))
		}
	}
}
...
func (s *ModelSuite) TestItGetsNotice() {
	for i := 0; i < s.n; i++ {
		...
		s.NoError(err)
		for j := 0; j < s.n; j++ {
			_, err := s.m.AddNotice([]byte(s.payloads[j]), common.HexToAddress(devnet.ApplicationAddress))
			s.Nil(err)
		}
		_, err = s.m.FinishAndGetNext(true) // finish
		s.NoError(err)
	...
	for i := 0; i < s.n; i++ {
		for j := 0; j < s.n; j++ {
			...
			s.Equal([]byte(s.payloads[j]), common.Hex2Bytes(notice.Payload[2:]))
		}
	}
}
...
func (s *ModelSuite) TestItGetsReport() {
	...
	for i := 0; i < s.n; i++ {
		...
		s.NoError(err)
		for j := 0; j < s.n; j++ {
			err := s.m.AddReport(common.HexToAddress(devnet.ApplicationAddress), []byte(s.payloads[j]))
			s.Nil(err)
		}
		_, err = s.m.FinishAndGetNext(true) // finish
		s.NoError(err)
...
func (s *ModelSuite) TestItGetsNumVouchers() {
	...
	for i := 0; i < s.n; i++ {
		...
		_, err = s.m.FinishAndGetNext(true) // get
		s.NoError(err)
		_, err = s.m.AddVoucher(common.HexToAddress(devnet.ApplicationAddress), s.senders[i], "0", []byte(s.payloads[i]))
		s.Nil(err)
		_, err = s.m.FinishAndGetNext(true) // finish
		s.Nil(err)
	}
...
func (s *ModelSuite) TestItGetsNumNotices() {
	...
	for i := 0; i < s.n; i++ {
		...
		_, err = s.m.FinishAndGetNext(true) // get
		s.NoError(err)
		_, err = s.m.AddNotice([]byte(s.payloads[i]), common.HexToAddress(devnet.ApplicationAddress))
		s.Nil(err)
		_, err = s.m.FinishAndGetNext(true) // finish
		s.Nil(err)
	}
...
func (s *ModelSuite) TestItGetsVouchers() {
	...
	for i := 0; i < s.n; i++ {
		for j := 0; j < s.n; j++ {
			...
			s.Equal([]byte(s.payloads[j]), common.Hex2Bytes(vouchers[idx].Payload[2:]))
		}
	}
}
...
func (s *ModelSuite) TestItGetsVouchersWithFilter() {
	...
	for i := 0; i < s.n; i++ {
		...
		s.Equal([]byte(s.payloads[i]), common.Hex2Bytes(vouchers[i].Payload[2:]))
	}
}
...
func (s *ModelSuite) TestItGetsNotices() {
	...
	for i := 0; i < s.n; i++ {
		for j := 0; j < s.n; j++ {
			...
			s.Equal([]byte(s.payloads[j]), common.Hex2Bytes(notices[idx].Payload[2:]))
		}
	}
}
...
func (s *ModelSuite) TestItGetsNoticesWithFilter() {
	...
	for i := 0; i < s.n; i++ {
		...
		s.Equal([]byte(s.payloads[i]), common.Hex2Bytes(notices[i].Payload[2:]))
	}
}
...
func (s *ModelSuite) TestItGetsReportsWithOffset() {
	...
	s.NoError(err)
	for i := 0; i < s.n*2; i++ {
		err := s.m.AddReport(common.HexToAddress(devnet.ApplicationAddress), []byte(s.payloads[i%s.n]))
		s.Nil(err)
	}
	_, err = s.m.FinishAndGetNext(true) // finish
	s.NoError(err)
